...
/* Create a modifer that checks if the msg.sender is the owner of the contract */
  ...
  modifier forSale(uint sku) {
    require(items[sku].state == State.ForSale, 'error');
    _;
  }

  modifier sold(uint sku) {
    require(items[sku].state == State.Sold, 'error');
    _;
  }
  modifier shipped(uint sku)  {
    require(items[sku].state == State.Shipped, 'error');
    _;
  }
  modifier received(uint sku) {
    require(items[sku].state == State.Received, 'error');
    _;
  }
